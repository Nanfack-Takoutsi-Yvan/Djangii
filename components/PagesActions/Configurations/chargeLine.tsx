...
import { fetchChargeLines } from "@services/store/slices/configurations/chargeLine"
import { chargeLineValidationForm } from "@services/validations/yup/pagesActions.validation"

const FormDefault = {
  designation: ""
}

const ChargeLineForm: FC<DashboardPagesCreationProps> = () => {
  ...
  const addPenaltyType = useCallback(
    (values: typeof FormDefault, { resetForm }: { resetForm: () => void }) => {
      if (association) {
        setLoading(true)
        const payload: ChargeLinePayload = {
          association,
          designation: values.designation
        }

        Charge.createChargeLine(payload)
          .then(() => {
            setActionModalProps({
              icon: true,
              state: "success",
    ...
    <View style={styles.singleMemberContainer}>
      <View style={styles.titleContainer}>
        <View style={styles.title}>
          <Text variant="titleLarge">{locale.t(`pages.newChargeLine`)}</Text>
        </View>
      ...
      <Formik
        validationSchema={chargeLineValidationForm}
