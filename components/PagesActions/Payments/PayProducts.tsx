...
import {
  fetchProductsTypes,
  getAllProductsTypes
} from "@services/store/slices/configurations/productTypes"
...
import { productPaymentValidation } from "@services/validations/yup/pagesActions.validation"
import ProductPayment from "@services/models/product/productPayment"
import { HttpStatusCode } from "axios"
import { fetchProductPaymentList } from "@services/store/slices/payment/productPayment"

const initialValues: ProductPaymentRequestBody = {
...
const PayProducts: FC = () => {
  ...
  const productList = getAllProductsTypes()
  ...
  const fetchProducts = useRef<boolean>(true)
  ...
  const submitForm = useCallback(
    async (
      values: ProductPaymentRequestBody,
      { resetForm }: { resetForm: () => void }
    ) => {
      setLoading(true)
      try {
        await ProductPayment.saveProductPayment(values)
        setActionModalProps({
          icon: true,
          state: "success",
          shouldDisplay: true,
        ...
        resetForm()
        if (associationId) {
          dispatch(fetchProductPaymentList(associationId))
        }
      } catch (err: any) {
        ...
        setActionModalProps({
          ...
          description: locale.t(
            error409
              ? "pages.productPaymentSavedConflict"
  ...
  useEffect(() => {
    if (associationId && fetchProducts.current) {
      dispatch(fetchProductsTypes(associationId))
      fetchProducts.current = false
    }
    if (productList.data) setSelectedProducts(productList.data)
  }, [associationId, dispatch, productList.data])
  ...
  if (productList.loading || memberList.loading) {
  ...
  return (
    <View>
      ...
      <Formik
        ...
        validationSchema={productPaymentValidation}
      >
        {({
          errors,
          touched,
            ...
            <View style={styles.field}>
              <View style={styles.screen}>
                <SelectDropdown
                  ...
                  defaultButtonText={locale.t("common.products")}
                  buttonStyle={{
                  ...
                  onSelect={(item: IProduct) => {
                    ...
                    setFieldValue("productId", item.id, true)
                  }}
                  search
